...
const GetFamilyProperties = ({familyProperties}) => {
    console.log(familyProperties);
    return (
        <div>
            <h1 className="text-2xl font-semibold my-3 text-blue-600">Family Properties</h1>
            <div className='grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-6'>
           {
            familyProperties?.map((data, idx) => 
            <div key={idx}>
            <div className="w-full cursor-pointer group hover:shadow-2xl hover:shadow-gray-400 overflow-hidden border bg-white rounded-lg shadow-lg dark:bg-gray-800">
                {
                data?.images ?
                <img className="object-cover group-hover:scale-110 transition duration-300 ease-in-out w-full h-20" src={data?.images} alt="avatar" />
                :
                <img className="object-cover group-hover:scale-110 transition duration-300 ease-in-out w-full h-20" src='https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT1m30CtcVbVnrNvkQHaTkhp-PX8AfmQJ_geunLB4ihtE5IwybGguPrQALKgLDaOZeCvIw&usqp=CAU' alt="avatar" />
                }
                    ...
                    <div className="p-2">
                        ...
                        <h1 className=" font-semibold text-gray-800 dark:text-white">District : {
                        data.district.length >= 10 ? <span>{data.district.slice(0,9)}...</span>
                        :
...
export default GetFamilyProperties;
